...
public abstract class PlayerAnimationMixin<T extends LivingEntity> {
	...
	public void setupPivot(T entityIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, CallbackInfo ci) {
		...
		if (entityIn instanceof Player player_)
			...
			return;
		PotNoodlesModPlayerAnimationAPI.PlayerAnimation animation = PotNoodlesModPlayerAnimationAPI.active_animations.get(player);
		if (animation == null)
			return;
	...
	public void setupAnim(T entityIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, CallbackInfo ci) {
		...
		if (!master.equals("pot_noodles")) {
			if (!PotNoodlesModPlayerAnimationAPI.animations.isEmpty())
				PotNoodlesModPlayerAnimationAPI.animations.clear();
			return;
		}
		PlayerModel<T> model = (PlayerModel<T>) (Object) this;
		Player player = null;
		...
		if (data.getBoolean("ResetPlayerAnimation")) {
			...
			PotNoodlesModPlayerAnimationAPI.active_animations.put(player, null);
		}
		...
		PotNoodlesModPlayerAnimationAPI.PlayerAnimation animation = PotNoodlesModPlayerAnimationAPI.active_animations.get(player);
		if (animation == null) {
			animation = PotNoodlesModPlayerAnimationAPI.animations.get(playingAnimation);
			if (animation == null) {
				PotNoodlesMod.LOGGER.info("Attepted to play null animation " + playingAnimation + ", did animations fail to load?");
				return;
			}
			PotNoodlesModPlayerAnimationAPI.active_animations.put(player, animation);
		}
		...
		for (Map.Entry<String, PotNoodlesModPlayerAnimationAPI.PlayerBone> entry : animation.bones.entrySet()) {
			String boneName = entry.getKey();
			PotNoodlesModPlayerAnimationAPI.PlayerBone bone = entry.getValue();
			ModelPart modelPart = getModelPart(model, boneName);
			if (modelPart == null)
				continue;
			// Apply rotation
			Vec3 rotation = PotNoodlesModPlayerAnimationAPI.PlayerBone.interpolate(bone.rotations, animationProgress, player);
			if (rotation != null) {
			...
			Vec3 position = PotNoodlesModPlayerAnimationAPI.PlayerBone.interpolate(bone.positions, animationProgress, player);
			if (position != null) {
			...
			// Apply scale
			Vec3 scale = PotNoodlesModPlayerAnimationAPI.PlayerBone.interpolate(bone.scales, animationProgress, player);
			if (scale != null) {
